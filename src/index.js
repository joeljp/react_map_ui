...
/*
// Old stuff
function Menu_(props){
    let comps = [];
    $.each(props.meta, function(k,v){
	if(["place","area","region","country","agegroup"].includes(k)){return true} // IE, geographical stuff, for the map.
	let nulls = false
	if(k == "age" || k == "birth" || k == "rec"){ // IE, integer -> ranges/inervals. These categories should be defined in config 
	    let min = 99999;
	    let max = 0;
	    $.each(v, function(i,e){
		if(i == "null"){
...
//		    <div>
			<Checkbox
			    key={k+"_"+i}
			    label={i}
//			    onChange={(e) => props.discrete([k,i,e.target.checked])}
			    onChange={(e) => props.discrete({cat: k,val: i,checked: e.target.checked})}
			/>
//		    </div>
		);
...
//	    comps.push(
//		<button key={"random_shit"} onClick={props.onClick}>
//			 {"hii"}
//		</button>
//		);
	}
